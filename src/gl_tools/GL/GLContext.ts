...
const DEFAULT_OPTIONS: GLContextOptions = { version: undefined };
const MAX_UNSIGNED_BYTE = 256 as const;
const MAX_UNSIGNED_SHORT = 65536 as const;
 ...
 */
function getContextUseLatest(canvas: HTMLCanvasElement, options?: WebGLContextAttributes): WebGL2RenderingContext | WebGLRenderingContext | undefined {
 ...
 * @returns A WebGL rendering context on success, otherwise undefined
 */
function getContextFailNotLatest(canvas: HTMLCanvasElement, options?: WebGLContextAttributes): WebGL2RenderingContext | undefined {
    let context: WebGL2RenderingContext | null = canvas.getContext("webgl2", options);
    if (context) return context;
    return undefined;
}
 ...
 * @returns A WebGL rendering context on success, otherwise undefined
 */
function getContextForceVOne(canvas: HTMLCanvasElement, options?: WebGLContextAttributes): WebGLRenderingContext | undefined {
    let context: WebGLRenderingContext | null = canvas.getContext("webgl", options);
    if (context) return context;
    return undefined;
}
...
export class GLContext {
     ...
     */
    static makeWebGLContext(canvas: HTMLCanvasElement, options?: WebGLContextAttributes) {
     ...
     */
    static initializeWithContext(canvas: HTMLCanvasElement, options: GLContextOptions = DEFAULT_OPTIONS, webGLOptions?: WebGLContextAttributes) {
        switch (options.version) {
            case undefined:
                const latestContext = getContextUseLatest(canvas, webGLOptions);
                if (latestContext) return new GLContext(latestContext);
                alert("Unable to get WebGL");
                return;
            case 2:
                const gl2Context = getContextFailNotLatest(canvas, webGLOptions);
                if (gl2Context) return new GLContext(gl2Context);
                alert("Unable to get WebGL2");
                return;
            case 1:
                const glContext = getContextForceVOne(canvas, webGLOptions);
                if (glContext) return new GLContext(glContext);
                alert("Unable to get WebGL");
                return;
        }
    }
    ...
    private readonly _version: GLVersion;
     ...
     */
    makeShaderProgram(vertexShader: VertexShader, fragmentShader: FragmentShader) {
        try {
            const program = new ShaderProgram(this.context, vertexShader, fragmentShader);
            return program;
        } catch (err) {
    ...
    packShape(shape: BaseShape) {
        ...
        this.context.bufferData(
            this.context.ARRAY_BUFFER,
            flattenVectors(shape.textureCoordinates()),
            this.context.STATIC_DRAW
        );
        ...
        let indexArray: Uint8Array | Uint16Array | Uint32Array | null = null;
        ...
        if (maxIndex < MAX_UNSIGNED_BYTE) {
            ...
            indexType = (this.context as WebGL2RenderingContext).UNSIGNED_INT;
        } else {
