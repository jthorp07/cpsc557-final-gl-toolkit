...
import { ArithmeticExpression, InvokationExpression, LiteralExpression, PropertyExpression, VariableExpression } from "../language/GLSLGrammar.js";
...
class GenericVertexShaderBuilder extends VertexShaderBuilder {
    constructor(version) {
        ...
        this.setMain(new VertexShaderMainBuilder(globalVariables).declareLocalVariable("vec4", "worldPosition", new ArithmeticExpression("vec4", new VariableExpression("mat4", "modelMatrix"), new InvokationExpression("vec4", "vec4", [
            new VariableExpression("vec3", "vertexPosition"),
            new LiteralExpression("float", PrimitiveLiteral.float(1.0))
        ]), "*")).assignGLPosition(new ArithmeticExpression("vec4", new ArithmeticExpression("mat4", new VariableExpression("mat4", "projectionMatrix"), new VariableExpression("mat4", "viewMatrix"), "*"), new VariableExpression("vec4", "worldPosition"), "*")).assignVariable("fragmentPosition", new InvokationExpression("vec3", "vec3", [
            new PropertyExpression("vec3", new VariableExpression("vec4", "worldPosition"), "xyz")
        ])).assignVariable("fragmentNormal", new InvokationExpression("vec3", "normalize", [
            new ArithmeticExpression("vec3", new VariableExpression("mat3", "normalMatrix"), new VariableExpression("vec3", "vertexNormal"), "*")
        ])).assignVariable("fragmentTextureCoordinate", new VariableExpression("vec2", "textureCoordinate")).assignVariable("fragmentColor", new VariableExpression("vec4", "vertexColor")));
    }
}
export function makeGenericVertexShader(version) {
