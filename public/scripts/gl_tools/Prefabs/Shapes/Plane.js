...
function planeVertices(width, height) {
    const dX = width / 2;
    const dY = height / 2;
    return [
        // Facing +Z
...
export class Plane extends BaseShape {
    static makeZPlane(width, height) {
        return new Plane(width, height);
    }
    static makeYPlane(width, height) {
        const plane = new Plane(width, height);
        plane.staticRotate(Y_PLANE_ROTATION);
        return plane;
    }
    static makeXPlane(width, height) {
        const plane = new Plane(width, height);
        plane.staticRotate(X_PLANE_ROTATION);
        return plane;
    }
    constructor(width, height) {
        super(planeVertices(width, height), planeNormals(), planeRelativeIndices(), planeTextureCoordinates());
    }
}
